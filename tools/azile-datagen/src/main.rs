...
#[command(
    name    = "nexus-datagen",
    about   = "Nexus AI MDM â€” Synthetic Entity Data Generator",
    version = "1.0.0",
...
struct Cli {
    ...
    #[arg(long, default_value = "azile-local-dev-token")]
    auth_token: String,

    /// Percentage of duplicate records to inject (0â€“50)
...
async fn main() -> Result<()> {
    ...
    if cli.duplicates > 50 {
        eprintln!("error: --duplicates must be 0â€“50");
        std::process::exit(1);
    }

    eprintln!(
        "âš¡  Generating {} {} records (seed={}, duplicates={}%)â€¦",
    ...
    eprintln!(
        "âœ“  {} base records + {} duplicates = {} total",
    ...
    match cli.output {
        ...
        OutputFormat::Api => {
            eprintln!("ðŸ“¤  Sending to {} in batches of {}â€¦", cli.api_url, cli.batch_size);
            eprintln!();
            ...
            if summary.total_failed > 0 {
                eprintln!("âš ï¸  {} records failed â€” check the ingest service logs.", summary.total_failed);
                std::process::exit(1);
            }
        }
    }
